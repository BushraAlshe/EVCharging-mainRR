...
def round_EV_scheduling_fixed_interval(ins, guess_x={}, round_x_after_y=True):
    ...
    if sol.succeed:
        ...
        the_best.obj = 0
        while not rounded_solution_feasibility or counting < 100:
            counting = counting + 1
            print("counting")
            print(counting)
            # matrix X1 containing the CN values
            ...
            for k in customers:
                if W1[k][k] > delta * 1000000 * 1000000:
            ...
            for k in customers:
                for c in ins.customer_charging_options[k]:
                    if 0.00001 < rounded_sol.x[k, c] < 1 - 0.00001:
                        rounded_sol.x[k, c] = 0
            # print(rounded_sol.x)
            ...
            for k in np.arange(ins.n):
                for c in ins.customer_charging_options[k]:
                    ...
                    X1.append(rounded_sol.x[k, c])
            obj1 = np.dot(np.transpose(z), X1)
            # print(sol.x)
            # print(rounded_sol.x)
            rounded_solution_feasibility = u.check_ev_scheduling_fixed_int_sol_feasibility(ins, rounded_sol,
                                                                                           fractional_sol=False)
            print(the_best.obj)
            if obj1 > the_best.obj:
                the_best = rounded_sol
                the_best.obj = obj1
...
def max_ev_scheduling_OPT_fixed_interval(ins, guess_x={}, fractional=False, debug=False, tolerance=0.001,
    ...
    for t in time_path:
        ...
        # print(d)
        m.addConstr(d, gbp.GRB.LESS_EQUAL, ins.capacity_over_time[t])
