...
def check_ev_scheduling_fixed_int_sol_feasibility(ins, sol, fractional_sol=False):
    ...
    for t1 in time_path:
        ...
        for k1 in np.arange(len(X1)):
            Q[0][k1] = 7000.0  # real value of S.
            Q[1][k1] = 0.0  # Imaginary value S.
        W1 = np.dot(np.transpose(Q), Q)
        rhs_P = np.dot(np.transpose(X1), W1)  # X^T . W
        rhs_P = np.dot(rhs_P, X1)  # X^T . W . X
        assert (rhs_P <= (
                ins.capacity_over_time[t1] - ins.base_load_over_time[t1])), "capacity violated at time %d" % t1
    energy_usage = {}
    for k in range(ins.n):
        energy_usage[k] = 0
        sum_of_x = 0
